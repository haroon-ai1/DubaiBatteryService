...
Static Assets. **v2 (Sep 2026): full redesign + Dubai local-SEO build-out — 55
indexable pages** (see "Site structure" below and `CHANGELOG-AUDIT.md`).
...
| Services hub + 7 services | `/services/`, `/car-battery-replacement-dubai/` … | `src/pages/*.astro` + `src/layouts/ServicePageLayout.astro` |
| 20 area pages + hub | `/car-battery-replacement-{area}/`, `/areas/` | `src/data/areas.ts` → `src/pages/car-battery-replacement-[area].astro` |
| 12 car-make pages + hub | `/{make}-battery-replacement-dubai/`, `/car-makes/` | `src/data/makes.ts` → `src/pages/[make]-battery-replacement-dubai.astro` |
| 6 guides + index | `/guides/{slug}/` | `src/data/guides.ts` + `src/pages/guides/*.astro` (`GuideLayout`) |
| Brands, about, contact, privacy, terms | `/car-battery-brands-dubai/` … | `src/layouts/ContentLayout.astro` |

**Adding an area / make:** add an entry to the data file with its OWN
written paragraphs (not a copy of another area's). The page, sitemap,
footer, hub and internal links all pick it up automatically.

**Adding a guide:** add an entry to `GUIDES` in `src/data/guides.ts`, then
create `src/pages/guides/{slug}.astro` using `GuideLayout`.

### Design system

Tokens in `src/styles/tokens.css` ("Night Shift": ink `#0b1017` + volt
`#ffd23f`, WhatsApp green reserved for WhatsApp). Shared primitives
(`.btn`, `.card`, `.section`, `.prose`, `.faq`, `.data-table`) live in
`src/styles/global.css`; page blocks in `src/components/blocks/`.
...
| `claims.*` (24/7, arrival 20–25 min, warranty up to 36 months, from AED 120, free testing, payment) | **set (Sep 2026, confirmed by current partner)** | Trust strip, hero bullets, sidebar, Service schema `hoursAvailable` / `offers`. ⚠️ Re-confirm with any new operator after a sale. |
