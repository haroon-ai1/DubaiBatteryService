# v2 — Redesign + Dubai local SEO (24 Sep 2026)

## Design
- New "Night Shift" design system: dark ink heroes + volt-yellow accent, card-based layout, floating trust strip, sticky desktop sidebar CTA, compact mobile cards, full-width tap targets on phones.
- New header (24/7 status strip, Services dropdown, Call + WhatsApp), rich footer (services / areas / makes / company), mobile sticky Call + WhatsApp bar.
- New OG image, favicon and touch icon in the new brand colours.

## SEO
- Existing 7 service URLs kept (no redirects needed) — retitled for commercial Dubai intent (24/7, AED price, arrival time).
- +20 area landing pages (`/car-battery-replacement-{area}/`) with area-specific copy, local FAQs and area-scoped `Service` schema.
- +12 car-make pages (`/{make}-battery-replacement-dubai/`) with typical batteries by model and registration notes.
- +6 guides with `Article` schema, + brands, services hub, areas hub, makes hub, about, contact, privacy, terms.
- `Service` schema now carries `hoursAvailable` (24/7) and an `AggregateOffer` (AED 120–2,000) from config.
- `hreflang="en-ae"`, geo meta, `max-image-preview:large`, `/llms.txt` for AI assistants.
- WhatsApp messages now include page context (area / car make) so the partner knows where the lead is.
- Every page: unique title ≤60 and description ≤155, one H1, canonical, valid JSON-LD, no orphans (enforced by `scripts/check-links.mjs`).

## Still to do (off-site — biggest ranking levers)
1. Google Search Console: verify the domain, submit `/sitemap-index.xml`, request indexing for home + service pages.
2. Google Business Profile: the local pack dominates "car battery dubai". A real GBP (service-area business) is needed; once it exists set `entityVerified`, `business.*` in `site.ts` to unlock LocalBusiness schema.
3. Citations/backlinks: UAE directories (Yellow Pages UAE, Connect.ae, etc.), and a few genuine local mentions.
4. Reviews: once real reviews exist, set `claims.reviewRating` / `reviewCount` — never before.
