...
export interface LeadContext {
  ...
  page?: string;
  /** Human area name from page context, e.g. "Dubai Marina". Not PII. */
  areaName?: string | null;
  /** Vehicle make from page context, e.g. "Toyota". Not PII. */
...
function composeMessage(
  ...
  const car = makeName ? ` My car is a ${makeName}.` : "";
  const where = areaName ? ` I'm in ${areaName}.` : "";
  return `${salutation}${attribution}${baseMessage(service)}${car}${where}`;
}
...
export function getWhatsAppUrl(ctx: LeadContext): string | null {
  ...
  const text = composeMessage(ctx.service, ctx.areaName, ctx.makeName);
